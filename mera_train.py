...
論文 §4 に基づく 2 モダリティ（画像→音声）の MERA パイプライン:
  ...
  Phase2  : θ_1 を出発点に音声モダリティを素直に FT（AudioCaps + Clotho-AQA）
             → θ_{2,vanilla}（= CKPT_AUDIO_VANILLA）
  ...
  Step2a  : CKPT_MERGED から画像コネクタを画像 replay で再整合
             → CKPT_IMG_REALIGNED/mm_projector.bin

  Step2b  : CKPT_MERGED から音声コネクタを音声 replay で再整合
             → CKPT_AUD_REALIGNED/mm_projector_a.bin（フルモデルも副産物として保存されるが使用しない）

  Assemble: CKPT_MERGED に両コネクタを注入して最終モデルを組み立てる
             → CKPT_FINAL
             ※ 評価③④: eval_vqa.py --model CKPT_FINAL --eval-json image_eval.json / audio_eval.json
                → 画像スコアC（BRG分子）・音声スコアD（FRG分子）

  BRG/FRG計算: calc_brg_frg.py を実行（評価①〜④をまとめて走らせて結果を出力）
...
def _apply_mera_step1_and_save(
    ...
    # θ_{i,vanilla}（音声を素直に FT したモデル）を in memory で読み込む
...
if __name__ == "__main__":
    ...
    train_image(
        ...
        output_dir   = config.CKPT_PHASE1,
        vision_tower = config.VISION_TOWER,
        num_gpus     = config.NUM_GPUS,
    )

    # ─────────────────────────────────────────────────────────────────────────
    # Phase 2: θ_1 を出発点に音声モダリティを素直に FT → θ_{2,vanilla}
    #   CL 手法なし・全パラメータ学習（これが MERA 論文の「vanilla model」）
    # ─────────────────────────────────────────────────────────────────────────
    print("\n" + "="*60)
    ...
    train_audio(
        ...
        output_dir      = config.CKPT_AUDIO_VANILLA,
        audio_tower     = config.BEATS_TOWER,
        lr              = 2e-4,   # 論文 Table 9: LLM・音声エンコーダの学習率
        mm_projector_lr = 2e-5,   # 論文 Table 9: 音声コネクタの学習率
        num_gpus        = config.NUM_GPUS,
        # tune_mm_mlp_adapter_a=False（デフォルト）→ LLM 含む全パラメータを学習
        # tune_audio_tower=True（デフォルト）→ 音声エンコーダも学習対象
    )
    ...
    #   出力: CKPT_IMG_REALIGNED/mm_projector.bin のみ
    # ─────────────────────────────────────────────────────────────────────────
    print("\n" + "="*60)
    ...
    train_image(
        ...
        vision_tower        = config.VISION_TOWER,
        tune_mm_mlp_adapter = True,   # LLM+ビジョンエンコーダを凍結、画像コネクタのみ学習
        lr                  = step2["lr"],
    ...
    #   出力: CKPT_AUD_REALIGNED/mm_projector_a.bin のみ
    # ─────────────────────────────────────────────────────────────────────────
    print("\n" + "="*60)
    ...
    train_audio(
        ...
        audio_tower           = config.BEATS_TOWER,
        tune_audio_tower      = False,  # 音声エンコーダを凍結
        tune_mm_mlp_adapter_a = True,   # LLM を凍結、音声コネクタのみ学習
        lr                    = step2["lr"],
        batch_size            = step2["batch_size"],
        grad_accum            = step2["grad_accum"],
        num_gpus              = config.NUM_GPUS,
